...
import { useGetVendorWaitingList } from '@/queries/super.queries';

type NavigationProp = StackNavigationProp<RootStackParamList, any>;

const VendorRequests = () => {
  const { layout, gutters, fonts, borders } = useTheme();
  const { data: vendorsData } = useGetVendorWaitingList();
  const navigation = useNavigation<NavigationProp>();

  const renderVendorCard = ({ item }: { item: any }) => (
    <RNBounceable
      onPress={() =>
        navigation.navigate(AdminPaths.VendorApproval, { vendor: item })
      }
      key={item._id}
      style={[
        gutters.padding_16,
